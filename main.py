...
def openDatasets():
    ...
    trainPts, validatePts, testPts = masterDataSet.sample(sampleEvenly=False)
...
def openAndTrain(train_dataset=None, val_dataset=None, test_dataset=None):
    ...
    predictions_result = example2(predictions, test, mod_string, dataset_fname)
    ...
    util.savePredictions(predictions, mod_string[7:])
    predictions_result = example2(predictions, real_test, mod_string, dataset_fname)
...
def example2(predictions, dataset, mod, training_data):
    ...
    results, predictions_result = viz.evaluate_heights(predictions, dataset)
...
def getRealHeights(dataset, all_preds=None):
    predictions = {}
    obj_height_list = []
    ...
    if all_preds is not None:
        cols = ['Location', 'y', 'x', 'prediction']
        preds = pd.read_csv(all_preds, names=cols)

    for count, pt in enumerate(ptList):
        locName, location = pt

        if count == 0 and classify: # or count == 1
            if count == 0:
            ...
            if all_preds is None:
                if classify or bin_class:
                    arg = np.argmax(loc.obj_height_classification[location])
                    if arg == 5: #fix this back to 1
                        obj_height_list.append(100)
                    else:
                ...
                mod = count%5000
                if mod == 0:
                    print("count: ", count, " / ", preds.shape[0])
                df1 = preds[(preds.y == location[0]) & (preds.x == location[1])]
                if classify or bin_class:
                    try:
                        pred_str = df1.prediction.item()
                        pred_lst = pred_str.strip('][').split(', ')
                        for i, val in enumerate(pred_lst):
                            pred_lst[i] = float(val)
                        pred_lst = np.array(pred_lst)
                        if np.argmax(pred_lst) == 5: #fix this back to 1
                            obj_height_list.append(100)
                        else:
                            obj_height_list.append(np.argmax(pred_lst)) #+ 1
                    except:
                        obj_height_list.append(0)
                else:
                    obj_height_list.append(float(df1.prediction.item()))

    resultDict = {pt:pred for (pt, pred) in zip(ptList, obj_height_list)}
    return resultDict


def example(preds=None):
    if sys.argv[1] == 'all':
        modfname = 'all'
        # file_loc = 'untrain'
    else:
    ...
    if modfname != 'all':
        ...
        if preds is None:
            ...
            predictions = getRealHeights(test, preds)
        print('got ground truth...')
        mod = 'all'

    predictions_result = example2(predictions, test, mod, datasetfname)
    # predictions = predictions_result #this is to show the wrong pixels
