...
def getModelAndTrain(masterDataSet, mod, test_set, load_datasets=False, save_mod=False):
    if mod is None:
        if SPLIT:
            ...
            mod.fit_generator(gen, val_gen, epochs=300)
...
# TODO: do transfer learning with small datasets after unsupervised pretraining... see how small the dataset can be
            ...
            mod.fit_generator(aug.flow([X_split_1, X_split_2], masterDataSet.trainy, batch_size=BS), validation_data=([val_split_1, val_split_2], masterDataSet.valy), steps_per_epoch=X_split_1.shape[0] // BS, epochs=EPOCHS, callbacks=[es])
        else:
            es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=20)
