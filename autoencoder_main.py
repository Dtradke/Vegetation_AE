...
def getModelAndTrain(masterDataSet, mod, test_set, load_datasets=False, save_mod=False):
    if mod is None:
        if SPLIT:
            ...
            X_split_1, X_split_2 = masterDataSet.trainX[:,:,:,:2], masterDataSet.trainX[:,:,:,3:]

            # val_split_1 = np.stack((masterDataSet.valX[:,:,:,0], masterDataSet.valX[:,:,:,2]), axis=3)
            # val_split_2 = masterDataSet.valX[:,:,:,3:]

            val_split_1, val_split_2 = masterDataSet.valX[:,:,:,:2], masterDataSet.valX[:,:,:,3:]
...
def modPredict(mod, masterDataSet):
    print("Predicting...")
    if SPLIT:
        X_split_1, X_split_2 = masterDataSet.testX[:,:,:,:2], masterDataSet.testX[:,:,:,3:]
