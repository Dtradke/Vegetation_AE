...
def getModelAndTrain(masterDataSet, mod, test_set, load_datasets=False, save_mod=False):
    if mod is None:
        if SPLIT:
            ...
            X_split_1 = np.column_stack((masterDataSet.trainX[:,:,:,0], masterDataSet.trainX[:,:,:,2]), axis=3) #np.stack((masterDataSet.trainX[:,:,:,3],masterDataSet.trainX[:,:,:,5:]), axis=3)#
            ...
            val_split_1 = np.column_stack((masterDataSet.valX[:,:,:,0], masterDataSet.valX[:,:,:,2]), axis=3) #np.stack((masterDataSet.trainX[:,:,:,3],masterDataSet.trainX[:,:,:,5:]), axis=3)#
