...
def getModelAndTrain(masterDataSet, mod, test_set, load_datasets=False, save_mod=False):
    if mod is None:
        if SPLIT:
            ...
            X_split_1 = np.column_stack((masterDataSet.trainX[:,:,:,0], masterDataSet.trainX[:,:,:,2])) #np.stack((masterDataSet.trainX[:,:,:,3],masterDataSet.trainX[:,:,:,5:]), axis=3)#
            ...
            val_split_1 = np.column_stack((masterDataSet.valX[:,:,:,0], masterDataSet.valX[:,:,:,2])) #np.stack((masterDataSet.trainX[:,:,:,3],masterDataSet.trainX[:,:,:,5:]), axis=3)#
...
def modPredict(mod, masterDataSet):
    ...
    if SPLIT:
        ...
        X_split_1 = np.column_stack((masterDataSet.testX[:,:,:,0], masterDataSet.testX[:,:,:,2])) #np.stack((masterDataSet.trainX[:,:,:,3],masterDataSet.trainX[:,:,:,5:]), axis=3)#
...
def regHeightsCheck(masterDataSet):
    ...
    for i, lower in enumerate(bottom):
        flat_train = flat_train[(flat_train >= lower) & (flat_train < top[i])]
        flat_val = flat_val[(flat_val >= lower) & (flat_val < top[i])]
        print("Height ", lower, " to ", top[i], " - train: ", flat_train.size, " - val: ", flat_val.size)
