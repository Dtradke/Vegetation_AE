...
def evaluateRegression(y_preds, masterDataSet):
    ...
    print("is ground nan: ", np.isnan(ground))
    print("is pred nan: ", np.isnan(y_preds))
    print("is ground inf: ", np.isinf(ground))
    print("is pred inf: ", np.isinf(y_preds))

    diff = np.absolute(np.subtract(ground, y_preds))
    print("is diff nan: ", np.isnan(diff))
    print("is diff inf: ", np.isinf(diff))
