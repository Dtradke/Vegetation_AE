...
def calcError(y_preds, ground, lower=0, upper=2):
    ...
    print("Confidence = mean: ", mean, " ... upper: ", (mean + (1.96*(std / np.sqrt(ground.size)))), " ... lower: ",  (mean - (1.96*(std / np.sqrt(ground.size)))), " ... added: ", (1.96*(std / np.sqrt(ground.size))), " ... 95% within: ", np.sort(diff)[int(diff.size * 0.9)])
